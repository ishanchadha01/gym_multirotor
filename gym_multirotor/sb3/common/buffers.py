...
from gym_multirotor.sb3.common.preprocessing import get_action_dim, get_obs_shape
from gym_multirotor.sb3.common.type_aliases import (
...
from gym_multirotor.sb3.common.vec_env import VecNormalize
...
class RolloutBuffer(BaseBuffer):
    ...
    def add(
        ...
        self.values[self.pos] = value.clone().cpu().detach().numpy().flatten()
        self.log_probs[self.pos] = log_prob.clone().cpu().detach().numpy()
        self.pos += 1
        if self.pos == self.buffer_size:
            self.full = True
